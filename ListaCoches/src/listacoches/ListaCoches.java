...
public class ListaCoches{
	...
	static void modificaciones(Scanner e,Coches c[]) {
		...
		do {
			do {
				...
				System.out.println("Matrícula del coche a modificar¿?");
				matricula=e.next();
			...
			for(i=0;i<c.length;i++) {
				...
				if(i!=5) {
					if(i==4)
						System.out.println("\n El coche con la matrícula "+matricula+" no existe.");
					else {
						do {
							do {
		...
		}while(otro=='s'&& i!=5); // si te quedaste sin objetos posibles a modificar.
	}		
	static void consultas(Coches c[]) {
		...
		do {
			...
			System.out.println("Matrícula del coche a consultar¿?");
