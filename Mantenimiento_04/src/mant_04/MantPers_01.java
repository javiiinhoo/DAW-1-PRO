...
public class MantPers_01{
	...
	void consultas()throws IOException{
		...
		do {
			System.out.println("Teclee Número de la persona: ");
			numBus=t.leerInt();
	...
	void modificaciones() throws IOException{
		...
		RandomAccessFile fich= new RandomAccessFile(ruta+"mantpers.dat","rw");
		do {
			//estoy cerrando y abriendo el fichero en cada modificación xd
			//el do puedes
			System.out.println("\n\tMODIFICACIONES\n\t==============");
			do {
				System.out.println("Teclee Número para modificar: ");
				numBus=t.leerInt();
			}while(numBus==Integer.MIN_VALUE);
			fich.seek(numBus*p.tamano());
			p.leerDeArchivo(fich);
	...
	void listado() throws IOException{
		...
		RandomAccessFile fich = new RandomAccessFile(ruta+"mantpers.dat","r");
		//		do {
		//		System.out.println("Teclee el número de lineas: ");
		...
		do{
			...
			if (cl == LINEAS){
				System.out.println("\nMedia de edad en la página: " + edadP/cl);
			...
			if(!fin){
				...
				System.out.println("Número\tNombre\t\t\t\tEdad");
				System.out.println("--------------------------------------------");
				cl=0;
			}
